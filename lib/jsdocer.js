...
const DEFAULT_PREFIX = `${DEFAULT_PARAM_NAME}.`;
 ...
 */
function parseIdentifierParam(param, prefix = '') {
  return [{ name: `${prefix}${param.name}`, type: DEFAULT_PARAM_TYPE }];
}
 ...
 * @returns {Array} Array of param objects with name and type
 */
function parseDefaultParam(param, prefix = '') {
  return [{
    name: `[${prefix}${param.left.name}=${param.right.value}]`,
 ...
 *
 * @return {Array} Array of param objects with name and type
 */
function parseDestructuredParam(param) {
  const props = param.properties;

  return props.reduce((params, { value }) => {
    if (value.type === 'Identifier') {
      return params.concat(parseIdentifierParam(value, DEFAULT_PREFIX));
    } else if (value.type === 'AssignmentPattern') {
      return params.concat(parseDefaultParam(value, DEFAULT_PREFIX));
    }
    return params;
  }, [{ name: DEFAULT_PARAM_NAME, type: 'Object' }]);
}
