...
function parseDestructuredParam(param) {
  let params = [{ name: DEFAULT_PARAM_NAME, type: 'Object' }];
  const props = param.properties;
  for (let i = 0; i < props.length; ++i) {
    const { value } = props[i];
    if (value.type === 'Identifier') {
      params = params.concat(parseIdentifierParam(value, DEFAULT_PREFIX));
    } else if (value.type === 'AssignmentPattern') {
      params = params.concat(parseDefaultParam(value, DEFAULT_PREFIX));
    }
  }
  return params;
}
